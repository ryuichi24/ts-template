...
const envs = {
  ...
  ELECTRON_APP_NAME: process.env.TST_ELECTRON_APP_NAME ?? "ts-template",
  ...
  getAppId() {
    return `${this.getReversedDomain()}.${this.ELECTRON_APP_NAME?.toLocaleLowerCase()}`;
  },
  getLowerCaseAppName() {
    return this.ELECTRON_APP_NAME.toLocaleLowerCase();
  },
  getCopyright() {
...
const ctx = {
  ...
  getElectronVersion() {
    ...
    const rawVersion =
      mainProcessModulePackageJson.devDependencies.electron ?? mainProcessModulePackageJson.dependencies.electron;
